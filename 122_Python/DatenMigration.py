...
def validate(line):
	for i,col in enumerate(line): # Go through each field in the line
		if not re.match(patterns[i]+'$', col): # If it fails then yield the value
			yield col
...
with open('Data1.csv', newline='') as f:
	...
	for lineNumber, line in enumerate(reader): # Go through every line in the csv
		...
		if not any(errors):
			...
			for error in errors:
				print(error or "leer") # print error or "leer" if empty
			print(ENDC) # End color and insert newline
